...
#[derive(Accounts)]
pub struct CreateProgrammableNFT<'info> {
    #[account(mut)]
...
#[derive(Accounts)]
pub struct CreateUtilityNFT<'info> {
    #[account(mut)]
    pub owner: Signer<'info>,
    #[account(
        init,
...
impl<'info> CreateProgrammableNFT<'info> {
    pub fn create_programmable_nft(
        &mut self, 
        bumps: &CreateProgrammableNFTBumps,
        name: String, 
        symbol: String, 
        uri: String,
        ...
        // Create creators with proper shares
        let creator_count = creators.len() as u8;
        let mut nft_creators = Vec::new();
        
        for creator in creators {
            nft_creators.push(Creator {
                address: creator,
                verified: false, // Will be verified separately
                share: 100 / creator_count,
            });
        }

        // Add the program authority as a creator
        nft_creators.push(Creator {
            address: self.mint_authority.key(),
            verified: true,
            share: if nft_creators.is_empty() { 100 } else { 0 },
        });

        // Create metadata with programmable features
        let metadata_account = CreateMetadataAccountV3Cpi::new(
            ...
            CreateMetadataAccountV3InstructionArgs {
                data: DataV2 {
                    ...
                    uses: None, // No usage limits for programmable NFTs
                },
                is_mutable: true, // Programmable NFTs should be mutable
                collection_details: None,
            }
        );
        metadata_account.invoke_signed(signer_seeds)?;
        msg!("Programmable NFT Metadata Account created!");
        ...
        msg!("Programmable NFT Master Edition Account created!");

        Ok(())
    }
}

impl<'info> CreateUtilityNFT<'info> {
    pub fn create_utility_nft(
        &mut self, 
        bumps: &CreateUtilityNFTBumps,
        name: String, 
        symbol: String, 
        uri: String,
        ...
        metadata_account.invoke_signed(signer_seeds)?;
        msg!("Utility NFT Metadata Account created!");
        ...
        master_edition_account.invoke_signed(signer_seeds)?;
        msg!("Utility NFT Master Edition Account created!");
